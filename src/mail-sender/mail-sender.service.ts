...
export class MailSenderService {
  ...
  async sendChangeEmailMail(
    ...
    const mailOptions = {
      from: `"${config.mail.senderCredentials.name}" <${config.mail.senderCredentials.email}>`,
      to: email, // list of receivers (separated by ,)
      subject: `Alterar o e-mail da sua conta ${config.project.name}`,
      html: mail,
    };
  ...
  async sendResetPasswordMail(
    ...
    const mailOptions = {
      from: `"${config.mail.senderCredentials.name}" <${config.mail.senderCredentials.email}>`,
      to: email, // list of receivers (separated by ,)
      subject: `Redefinir a senha da sua conta ${config.project.name}`,
      html: mail,
    };
  ...
  async sendPasswordChangeInfoMail(
    ...
    const mail = changePasswordInfo
      ...
      .replace(new RegExp('--ProjectLink--', 'g'), config.project.url)
      .replace(new RegExp('--Socials--', 'g'), this.socials)
      .replace(new RegExp('--ContactEmail--', 'g'), config.project.email)
      .replace(new RegExp('--ButtonLink--', 'g'), buttonLink);

    const mailOptions = {
      from: `"${config.mail.senderCredentials.name}" <${config.mail.senderCredentials.email}>`,
      to: email, // list of receivers (separated by ,)
      subject: `A senha da sua conta ${config.project.name} foi alterada`,
      html: mail,
    };
